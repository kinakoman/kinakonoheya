...
class NewClass implements Base1, Base2 {
    ...
    public void goodbye() {
        System.out.println("goodbye");
                    ...
                    <Sub>インターフェースの継承</Sub>
                    <Tx>クラスと同様にインターフェースも継承が可能です。implements後のクラスでは継承元のいインターフェースメソッドも切っ述しなければエラーになります。</Tx>
                    <Code lang="java">{`interface Base1 {
    public void hello();
}
...
interface Base2 extends Base1 {
    public void goodbye();
                ...
                <Sub>コレクションとは</Sub>
                <Tx>コレクションは配列などのオブジェクトの集合を扱う処理が定義されたクラスを指します。
                    コレクションにはListやSetなどのインターフェースが実装されており、各インターフェースに固有のメソッドが実装されています。
                </Tx>
